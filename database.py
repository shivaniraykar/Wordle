...
class databaseLogger:
    def __init__(self) -> None:
        ...
        self.cur.execute('''CREATE TABLE attemptTable
               (id integer primary key autoincrement, attempt text, userWord text, gameWord text, success bool,  gameId integer, FOREIGN KEY(gameDataID) REFERENCES game(id))''')
        self.cur.execute('''CREATE Table statistics(id integer primary key autoincrement, gamesPlayed integer, game1Distribution integer,  game2Distribution integer, game3Distribution integer, game4Distribution integer, game5Distribution integer, game6Distribution integer, winPercentage integer, gameId integer, FOREIGN KEY(gameDataID) REFERENCES game(id))''')
        self.ip = self.getIp()
